...
function makeGuestId(name) {
  const slug = String(name || 'guest').toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '').slice(0, 32);
...
r.post('/', async (req, res, next) => {
  try {
    const { id, name, email, phone, nationality, idNumber, passport, vip, stays, notes, lastStay } = req.body;
    if (!name) return res.status(400).json({ error: 'name required' });
    const guestId = id || makeGuestId(name);
    await pool.execute('INSERT INTO guests (id,name,email,phone,nationality,idNumber,vip,stays,notes,lastStay) VALUES (?,?,?,?,?,?,?,?,?,?)',
      [guestId, name, email ?? null, phone ?? null, nationality ?? null, idNumber ?? passport ?? null,
       vip ? 1 : 0, stays ?? 0, JSON.stringify(notes ?? []), lastStay ?? null]);
    const [[row]] = await pool.query('SELECT * FROM guests WHERE id = ?', [guestId]);
...
r.put('/:id', async (req, res, next) => {
  try {
    ...
    const { name, email, phone, nationality, idNumber, passport, vip, stays, notes, lastStay } = req.body;
    await pool.execute('UPDATE guests SET name=?,email=?,phone=?,nationality=?,idNumber=?,vip=?,stays=?,notes=?,lastStay=? WHERE id=?',
      [name ?? existing.name, email ?? existing.email, phone ?? existing.phone,
       nationality ?? existing.nationality, idNumber ?? passport ?? existing.idNumber,
